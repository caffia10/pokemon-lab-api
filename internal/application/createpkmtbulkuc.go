...
func (uc *defaultCreatePokemonTypeInBulkUseCase) Do(pkmst []domain.PokemonType) error {

	var wg sync.WaitGroup
	errChannel := make(chan *domain.PokemonType)
	for _, pkmt := range pkmst {
		wg.Add(1)
		go func(pt *domain.PokemonType) {
			defer wg.Done()
