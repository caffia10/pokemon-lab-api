...
type CreatePokemonType struct {
	s      application.CreatePokemonTypeUseCase
	logger *zap.Logger
}

func (c *CreatePokemonType) Validate(ctx *fiber.Ctx) error {
...
func (c *CreatePokemonType) Handle(ctx *fiber.Ctx) error {
	...
	errC := c.s.Do((*domain.PokemonType)(pkmd))
...
func NewCreatePokemonTypeController(s application.CreatePokemonTypeUseCase, logger *zap.Logger) *CreatePokemonType {
