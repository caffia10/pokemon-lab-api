...
func (r *PokemonMongo) RetriveById(id string) (*domain.Pokemon, error) {
	...
	for _, v := range pkmDto.Types {
		pkm.Types = append(pkm.Types, domain.PokemonType{Id: v})
	}
...
func (r *PokemonMongo) initializeDtoFromModel(pkm *domain.Pokemon) *pokemonDto {
	...
	pkmd := &pokemonDto{
		...
		Weight:   pkm.Weight,
		ImgUrl:   pkm.ImgUrl,
		Types:    make([]string, len(pkm.Types)),
	}
	...
	for _, v := range pkm.Types {
		...
		pkmd.Types = append(pkmd.Types, oId.String())
	}
...
func NewPokemonMongoRepository(cm *mongo.Database, c *config.Config, logger *zap.Logger) domain.PokemonRepository {
