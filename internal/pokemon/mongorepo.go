...
func (r *PokemonMongo) RetriveById(id string) (*Pokemon, error) {
	...
	for _, v := range pkmDto.Types {
		pkm.Types = append(pkm.Types, &PokemonType{Id: v.String()})
	}

	return pkm, nil
}

func (r *PokemonMongo) initializeDtoFromModel(pkm *Pokemon) *pokemonMongoDto {

	pkmd := &pokemonMongoDto{
...
func (r *PokemonMongo) Create(pkm *Pokemon) error {
...
func (r *PokemonMongo) CreateManyPokemon(pkms []*Pokemon) error {
...
func NewPokemonMongoRepository(cm *mongo.Database, c *config.Config, logger *zap.Logger) PokemonRepository {
